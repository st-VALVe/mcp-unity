...
namespace McpUnity.Tools
{
    ...
    public class RunTestsTool : McpToolBase
    {
        ...
        {
            // async void: exceptions thrown after the first await would not surface to the
            // socket handler's synchronous try/catch and would leave tcs uncompleted, causing
            // the Node caller to hit a 30s timeout instead of getting a structured error.
            // Wrap the whole body and use TrySetResult so we are safe under any throw point
            // (including EditorSceneManager.OpenScene/SaveScene paths inside preflight).
            try
            {
            ...
            {
                McpLogger.LogError($"Error executing tool {Name}: {ex.Message}\n{ex.StackTrace}");
