...
const toolDescription = "Invokes a UnityEvent on a component of the target GameObject (e.g. Button.onClick, Toggle.onValueChanged, InputField.onEndEdit). " +
...
export function registerInvokeUIEventTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to invoke ${params.eventName} on ${params.objectPath}`);
    }
    return {
        content: [
            {
