...
const toolDescription = 'Legacy alias for capture_game_view. Captures a PNG screenshot of the Unity Game view and returns the absolute file path.';
const paramsSchema = z.object({
    outputPath: z
        .string()
        .optional()
        .describe("Optional output path for the PNG. Absolute or relative to the Unity project root. Defaults to 'Temp/mcp-screenshots/screenshot_<utc>.png'."),
    superSize: z
        .number()
        .int()
        .min(1)
 ...
 */
export function registerScreenshotTool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || 'Failed to capture screenshot');
    }
    return {
        content: [
            {
