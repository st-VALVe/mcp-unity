...
export function registerUiTools(server, mcpUnity, logger) {
    for (const registration of registrations) {
        logger.info(`Registering tool: ${registration.name}`);
        server.tool(registration.name, registration.description, registration.schema.shape, async (params) => {
            try {
...
async function uiToolHandler(mcpUnity, toolName, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `Failed to execute Unity UI tool '${toolName}'`);
    }
    return {
        content: [{
                type: 'text',
