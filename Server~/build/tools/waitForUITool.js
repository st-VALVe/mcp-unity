...
const toolDescription = "Polls for a GameObject by hierarchy path until it appears, is active, and (optionally) is interactable, " +
...
const paramsSchema = z.object({
    ...
    requireInteractable: z
        ...
        .optional()
        .default(true)
        .describe("If true (default), require the GameObject to be active AND not blocked by a CanvasGroup AND its " +
        "Selectable (if any) to be interactable. If false, only checks active state."),
});
export function registerWaitForUITool(server, mcpUnity, logger) {
...
async function toolHandler(mcpUnity, params) {
    ...
    if (!response.success) {
        throw new McpUnityError(ErrorType.TOOL_EXECUTION, response.message || `wait_for_ui failed for '${params.objectPath}'`);
    }
    return {
        content: [
            {
