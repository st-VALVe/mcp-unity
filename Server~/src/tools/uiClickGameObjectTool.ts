...
const toolDescription =
  "Performs an honest pointer click on a GameObject through the EventSystem. " +
...
const paramsSchema = z.object({
  objectPath: z
    ...
    .min(1)
    .describe(
      "Hierarchy path or leaf name of the GameObject to click, e.g. 'Canvas/Panel/StartButton'. " +
      "Includes inactive objects in the search, but a click is only dispatched if the target is active and not occluded."
    ),
});

export function registerUIClickGameObjectTool(server: McpServer, mcpUnity: McpUnity, logger: Logger) {
...
async function toolHandler(
  ...
  if (!response.success) {
    throw new McpUnityError(
      ...
      response.message || `Failed to click '${params.objectPath}'`
    );
  }
