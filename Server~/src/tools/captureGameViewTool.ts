...
const toolDescription =
  'Captures a PNG screenshot of the Unity Game view and returns the absolute file path plus scene/play-mode metadata. Useful for preserving visual evidence during UI and FTUE tests.';
...
const paramsSchema = z.object({
  outputPath: z
    ...
    .optional()
    .describe(
      "Optional output path for the PNG. Absolute or relative to the Unity project root. Defaults to 'Temp/mcp-screenshots/game_view_<utc>.png'."
    ),
  superSize: z
    .number()
    .int()
...
async function toolHandler(
  ...
  if (!response.success) {
    throw new McpUnityError(
      ...
      response.message || 'Failed to capture Unity Game view'
    );
  }
