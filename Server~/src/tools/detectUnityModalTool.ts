...
import { UnityModalHelper, DetectResult } from '../utils/unityModalHelper.js';

const toolName = 'detect_unity_modal';
const toolDescription =
  'Out-of-process detection of native Win32 modal dialogs blocking the Unity Editor main thread. Read-only — never clicks anything. Use when WebSocket is connected but Unity stops responding to tool calls. Windows-only.';
...
export function registerDetectUnityModalTool(
  server: McpServer,
  _mcpUnity: McpUnity,
...
function summarize(result: DetectResult): string {
  if (result.found) {
