...
const toolDescription =
  'Captures a diagnostic artifact bundle for UI/test failures: Game view screenshot, console logs, scene hierarchy, and metadata. Returns paths instead of huge payloads.';
...
const paramsSchema = z.object({
  ...
  outputDir: z
    ...
    .optional()
    .describe(
      "Optional parent output directory. Absolute or relative to the Unity project root. Defaults to 'Temp/mcp-diagnostics'."
    ),
...
export function registerCaptureDiagnosticsTool(server: McpServer, mcpUnity: McpUnity, logger: Logger) {
...
async function toolHandler(
  ...
  if (!response.success) {
    throw new McpUnityError(
      ...
      response.message || 'Failed to capture Unity diagnostics'
    );
  }
