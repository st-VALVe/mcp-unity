...
import { UnityModalHelper, DismissResult } from '../utils/unityModalHelper.js';

const toolName = 'dismiss_unity_modal';
const toolDescription =
  'Out-of-process dismissal of a native Win32 modal dialog blocking the Unity Editor main thread. Caller must specify the exact button name (case-sensitive). Idempotent — returns dialog_already_dismissed if the dialog is gone. Windows-only.';
...
interface UnityResponsivenessProbe {
  wsConnected: boolean;
  mainThreadResponsive?: boolean;
  probeMs?: number;
}

interface DismissToolResponse extends DismissResult {
  unityResponsiveAfter: UnityResponsivenessProbe;
}

export function registerDismissUnityModalTool(
  server: McpServer,
  mcpUnity: McpUnity,
  helper: UnityModalHelper,
  logger: Logger
) {
...
async function toolHandler(
  ...
  const response: DismissToolResponse = { ...dismissResult, unityResponsiveAfter };
...
async function probeUnityResponsiveness(mcpUnity: McpUnity): Promise<UnityResponsivenessProbe> {
...
function summarize(r: DismissToolResponse): string {
...
function formatProbe(p: UnityResponsivenessProbe): string {
  if (!p.wsConnected) return 'WebSocket not connected — cannot probe Unity responsiveness.';
  if (p.mainThreadResponsive) return `Unity main thread responsive (${p.probeMs}ms).`;
