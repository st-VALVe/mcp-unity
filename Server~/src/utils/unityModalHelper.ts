...
export interface UnityProcessInfo {
  ...
  topLevelWindows: Array<{
    ...
    buttons?: Array<{ name: string; automationId: string | null }>;
  }>;
}
...
export interface ModalDialog {
  ...
  windowClass: string;
  isNative: true;
  availableButtons: Array<{ name: string; automationId: string | null }>;
}

export interface DetectFoundResult {
  found: true;
  targetProcess: { pid: number; mainWindowTitle: string; projectPath: string | null };
  dialog: ModalDialog;
}
...
export interface DetectNotFoundResult {
  ...
  targetProcess: { pid: number; mainWindowTitle: string; projectPath: string | null };
  unsupportedDialog?: {
    topLevelWindows: Array<{ title: string; className: string }>;
  };
}
...
const realPathExists = async (p: string): Promise<boolean> => {
  try {
    await fs.access(p);
    return true;
  } catch {
...
const defaultEnvironment = (): HelperEnvironment => ({
  spawn: nodeSpawn,
  env: process.env,
  cwd: () => process.cwd(),
...
interface SnapshotOk {
  ok: true;
  processes: UnityProcessInfo[];
}
interface ClickOk {
  ok: true;
  outcome: 'clicked' | 'no_target_window' | 'button_not_available';
  availableButtons?: Array<{ name: string; automationId: string | null }>;
}
interface PsErr {
  ok: false;
  kind: 'uac_mismatch' | 'internal';
  message?: string;
}

type SnapshotResponse = SnapshotOk | PsErr;
type ClickResponse = ClickOk | PsErr;
 ...
 */
function isLikelyEditorInstance(p: UnityProcessInfo): boolean {
  return (p.mainWindowTitle ?? '').trim() !== '';
}

export class UnityModalHelper {
  private readonly logger: Logger;
  private readonly envCtx: HelperEnvironment;
  ...
  async detect(options: DetectOptions = {}): Promise<DetectResult> {
    ...
    return this.withInflightGuard(async () => {
      ...
      const extras = target.topLevelWindows.filter(
        (w) => w.className !== UNITY_MAIN_WINDOW_CLASS && w.className !== NATIVE_DIALOG_CLASS
      );
      if (extras.length > 0) {
        return {
          found: false,
  ...
  async dismiss(options: DismissOptions): Promise<DismissResult> {
    this.assertWindows();
    return this.withInflightGuard(async () => {
      ...
      const click = await this.runClick(
        target.pid,
        options.button,
        options.budgetMs ?? DEFAULT_DISMISS_BUDGET_MS
      );
  ...
  private async withInflightGuard<T>(fn: () => Promise<T>): Promise<T> {
    if (this.inflight) {
  ...
  private async runSnapshot(budgetMs: number): Promise<SnapshotOk> {
    const raw = await this.spawnPs<SnapshotResponse>({ action: 'snapshot' }, budgetMs, 'snapshot');
    if (!raw.ok) {
  ...
  private async runClick(targetPid: number, button: string, budgetMs: number): Promise<ClickOk> {
    const raw = await this.spawnPs<ClickResponse>(
      { action: 'click', targetPid, button },
      budgetMs,
      'click'
    );
    if (!raw.ok) {
  ...
  private mapPsError(err: PsErr): McpUnityError {
    ...
    return new McpUnityError(
      ErrorType.INTERNAL,
      err.message ? `modal_helper_internal: ${err.message}` : 'modal_helper_internal'
    );
  }

  private spawnPs<T>(payload: object, budgetMs: number, action: string): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      ...
      try {
        ...
        reject(
          new McpUnityError(
            ErrorType.INTERNAL,
            `modal_helper_spawn_failed: ${(e as Error).message}`
          )
        );
        return;
      }
      ...
      const settle = (err?: McpUnityError, value?: T) => {
        if (settled) return;
        settled = true;
      ...
      const budgetTimer = setTimeout(() => {
        ...
        settle(
          new McpUnityError(
            ErrorType.TIMEOUT,
            `modal_helper_timeout_${action} budget=${budgetMs}ms`
          )
        );
        try {
          proc.kill('SIGTERM');
      ...
      proc.on('close', () => {
        ...
        try {
          ...
          settle(
            new McpUnityError(
              ErrorType.INTERNAL,
              `modal_helper_parse_failed_${action}: ${(e as Error).message}`,
      ...
      try {
        ...
        settle(
          new McpUnityError(
            ErrorType.INTERNAL,
            `modal_helper_stdin_failed: ${(e as Error).message}`
          )
        );
      }
    });
  ...
  private async resolveTarget(
    ...
    throw new McpUnityError(
      ...
      { candidates: editors.map((p) => this.candidateInfo(p)) }
    );
  }
  ...
  private matchByProjectPath(
    ...
    const matches = processes.filter(
      (p) => p.projectPath && normalizePathForCompare(p.projectPath) === norm
    );
    if (matches.length === 0) {
      throw new McpUnityError(
        ErrorType.VALIDATION,
        fromCwd ? 'no_unity_process_for_cwd' : 'no_unity_process_for_project',
        {
          projectPath,
          candidates: processes.map((p) => this.candidateInfo(p)),
        }
    ...
    if (matches.length > 1) {
      throw new McpUnityError(
        ErrorType.VALIDATION,
        'multiple_unity_processes_require_explicit_target',
        { candidates: matches.map((p) => this.candidateInfo(p)) }
      );
    }
    return matches[0];
  }

  private candidateInfo(p: UnityProcessInfo) {
 ...
 */
export async function attachModalDiagnosticsOnTimeout(
  helper: UnityModalHelper | null | undefined,
  enabled: boolean,
  budgetMs: number,
  error: { details?: any }
): Promise<void> {
  ...
  try {
    const winner: DetectResult | null = await Promise.race([
      helper.detect({ budgetMs }).catch(() => null as DetectResult | null),
